...
```
DR(Designated Router) - R2
BDR(Backup Designated Router) - R1
Выбор DR и BDR осуществляется на основе приоритета OSPF интерфейса. 
Так как приоритеты интерфейсов не были изменены и равны значению по умолчанию (1),
выбор был произведён по наибольшему значению Router ID.
Маршрутизатор R2 имеет Router ID 2.2.2.2, что больше чем Router ID маршрутизатора R1 (1.1.1.1),
поэтому R2 был выбран в качестве DR а R1 BDR.
```
...
```
```
Passive-interface предотвращает формирование соседств на интерфейсе, но не запрещает распространение маршрута. Поэтому маршрут Loopback1 продолжает объявляться по OSPF.
```
