...
```
На коммутаторе S2 корневым является порт Fa0/4
На коммутаторе S3 корневым является порт Fa0/2
```
Какие порты на коммутаторе являются назначенными портами?
```
На коммутаторе S1 назначенными портами являются порт Fa0/4 и Fa0/2
На коммутаторе S3 назначенным портом является порт Fa0/4
```
Какой порт отображается в качестве альтернативного и в настоящее время заблокирован?
```
На коммутаторе S2 порт Fa0/2 альтернативный и заблокирован
```
Почему протокол spanning-tree выбрал этот порт в качестве невыделенного (заблокированного) порта?
```
Порт S2 Fa0/2 был выбран STP как невыделенный и переведён в состояние заблокированного, потому что между S2 и S3 два равнозначных пути к корневому коммутатору S1 (стоимость пути у обоих 19). При равной стоимости STP выбирает назначенный порт по меньшему Bridge ID.
```
